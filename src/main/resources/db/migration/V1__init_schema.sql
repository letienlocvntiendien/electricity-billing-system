...
CREATE TABLE billing_period (
    ...
    status              ENUM('OPEN', 'READING_DONE', 'CALCULATED', 'APPROVED', 'CLOSED') NOT NULL DEFAULT 'OPEN',
    approved_by         BIGINT,
...
-- ----------------------------------------------------------------
-- 5. meter_reading
-- read_at IS NULL means the reading has not yet been submitted by METER_READER.
-- When creating a period, rows are pre-populated with current_index = previous_index and read_at = NULL.
-- ----------------------------------------------------------------
CREATE TABLE meter_reading (
    ...
    KEY idx_reading_read_at (read_at),
...
CREATE TABLE bill (
    ...
    status              ENUM('PENDING', 'SENT', 'PARTIAL', 'PAID', 'OVERDUE') NOT NULL DEFAULT 'PENDING',
